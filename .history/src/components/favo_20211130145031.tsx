...
const Favo = () => {
  ...
  useEffect(() => {
    data.favos.map((value: any, i: any) => {
      const newFavoList = value.movieId;

      console.log(newFavoList);

      fetch(
        `https://api.themoviedb.org/3/movie/${newFavoList}?api_key=78eb03987f34d05d918381d81d8257fe&language=ja`
      )
        .then((response) => {
        ...
        .then((firstdata) => {
          ...
          movies.push(newFirstlist);
          const newMovie = movies;
          setFavodata({ ...favodata, newMovie });
  ...
  if (loading) {
    ...
    console.log(favodata);
    console.log(movies);
    const newfavodata = favodata;
    console.log(newfavodata);
    return (
      <>
        <div>
          <div className="row" style={{ margin: "0" }}>
            ...
            >
              {favodata.newMovie.length === 0
                ? favodata.newMovie.map((list: any, i: any) => {
                    return (
